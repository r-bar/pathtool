# Sort and deduplicate $PATH entries
...
between our interactive and non-interactive shells.

The utility that does the `$PATH` fuckery is `/usr/libexec/path_helper`. From
what I can tell it has no options other than to output either `sh` or `csh`
compatible syntax with the `-s` and `-c` flags respectively. It is invoked by
